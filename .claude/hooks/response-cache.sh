...
# Hash tool+input for cache key — используем sha1 внутри Python
# (без truncation: разные команды с одинаковым началом — разные хэши)
CALL_HASH=$(echo "$1" | python3 -c "
import sys, json, hashlib
d = json.load(sys.stdin)
key = d.get('tool','') + '|' + json.dumps(d.get('tool_input',{}), sort_keys=True)
...
if [ -f "$RESULTS_DIR/$CALL_HASH.txt" ]; then
  ...
  # Update bash_chain counter for anti-loop-guard before blocking
  # response-cache.sh exit 1 означает что anti-loop-guard.sh НЕ запустится,
  # поэтому обновляем счётчик здесь, чтобы Bash chain не потерялся.
  if [ "$TOOL" = "Bash" ]; then
    BCHAIN_FILE="/tmp/claude_antiloop/bash_chain"
    mkdir -p "$(dirname "$BCHAIN_FILE")"
    BCHAIN=$(cat "$BCHAIN_FILE" 2>/dev/null || echo 0)
    BCHAIN=$((BCHAIN + 1))
    echo "$BCHAIN" > "$BCHAIN_FILE"
  fi
