...
# Запрещает tool→tool подряд, если не выполнены исключения
# Разрешённые паттерны: Read→Bash, Bash→Read, Write→Bash, однотипные git
ONE_SHOT_FILE="$STATE_DIR/one_shot_last_tool_step"
ONE_SHOT_EXEMPT_FILE="$STATE_DIR/one_shot_exempt"

# Запоминаем предыдущий шаг для следующего вызова
if [ -f "$ONE_SHOT_FILE" ]; then
  LAST_TOOL_STEP=$(cat "$ONE_SHOT_FILE" 2>/dev/null || echo 0)
  LAST_PREV_TOOL=$(cat "$STATE_DIR/one_shot_tool_name" 2>/dev/null || echo "")
  EXEMPT=$(cat "$ONE_SHOT_EXEMPT_FILE" 2>/dev/null || echo "0")

  if [ "$LAST_TOOL_STEP" -ge 1 ] && [ "$STEP" -gt "$LAST_TOOL_STEP" ] && [ "$STEP" -le $((LAST_TOOL_STEP + 1)) ] && [ "$EXEMPT" != "1" ]; then
    # Разрешённые пары: не блокируем полезные последовательности
    IS_SAFE_PAIR=0

    # Read → Bash: прочитал → выполни
    if echo "$LAST_PREV_TOOL" | grep -qE "^Read$" && echo "$TOOL" | grep -qE "^Bash"; then
      IS_SAFE_PAIR=1
    fi

    # Bash → Read: выполни → проверь результат
    if echo "$LAST_PREV_TOOL" | grep -qE "^Bash" && echo "$TOOL" | grep -qE "^Read$"; then
      IS_SAFE_PAIR=1
    fi

    # Write → Bash: написал → протестируй
    if echo "$LAST_PREV_TOOL" | grep -qE "Write|Edit" && echo "$TOOL" | grep -qE "^Bash"; then
      IS_SAFE_PAIR=1
    fi
    ...
    # # npm/npx → Bash: установка → запуск
    if echo "$LAST_PREV_TOOL" | grep -qE "Bash\(npm|Bash\(npx" && echo "$TOOL" | grep -qE "^Bash"; then
      IS_SAFE_PAIR=1
    fi
...
# Reset exempt flag after check
echo "0" > "$ONE_SHOT_EXEMPT_FILE"

# Пропускаем cheap tools, не записываем их как триггер one-shot
CHEAP_TOOLS="Bash\(ls|Bash\(echo|Bash\(cat|Bash\(pwd|Bash\(which|Bash\(git status|Bash\(git diff"
if ! echo "$TOOL" | grep -qE "$CHEAP_TOOLS" 2>/dev/null; then
  echo "$STEP" > "$ONE_SHOT_FILE"
  echo "$TOOL" > "$STATE_DIR/one_shot_tool_name"
fi
...
# === CONTEXT BUDGET (новое) ===
# Отслеживает накопленный объём прочитанных данных в контексте.
...
if [ "$TOOL" = "Read" ]; then
  # Извлекаем offset и limit из tool_input
  READ_OFFSET=$(echo "$TOOL_INPUT" | python3 -c "import sys,json; print(json.load(sys.stdin).get('offset',0))" 2>/dev/null || echo 0)
  READ_LIMIT=$(echo "$TOOL_INPUT" | python3 -c "import sys,json; print(json.load(sys.stdin).get('limit',0))" 2>/dev/null || echo 0)

  # Если limit не указан — оцениваем через размер файла
  if [ "$READ_LIMIT" -le 0 ]; then
    TOTAL_LINES=$(wc -l < "$FILE_PATH" 2>/dev/null || echo 100)
    READ_LIMIT=$((TOTAL_LINES - READ_OFFSET))
    if [ "$READ_LIMIT" -le 0 ]; then READ_LIMIT=50; fi
  fi

  # Аппроксимация: ~50 байт на строку кода
  ESTIMATED_BYTES=$((READ_LIMIT * 50))
  if [ "$ESTIMATED_BYTES" -le 0 ]; then ESTIMATED_BYTES=500; fi

  CTX_USED=$(cat "$CONTEXT_BUDGET_FILE" 2>/dev/null || echo 0)
  CTX_NEW=$((CTX_USED + ESTIMATED_BYTES))
  echo "$CTX_NEW" > "$CONTEXT_BUDGET_FILE"

  if [ "$CTX_NEW" -gt "$MAX_CONTEXT_BYTES" ]; then
    echo "HARD STOP: context budget exceeded (~${CTX_NEW}b > ${MAX_CONTEXT_BYTES}b)." >&2
    echo "   Слишком много данных прочитано (~$((CTX_NEW/50)) строк). Ответь с тем, что уже есть." >&2
    exit 1
  fi
fi

# Сброс контекстного бюджета на новом промпте (phase reset уже делает частичный сброс)
# При смене фазы — сбрасываем контекстный бюджет вдвое
if [ "$CURRENT_PHASE" != "$PREV_PHASE" ] && [ -f "$CONTEXT_BUDGET_FILE" ]; then
  CTX_USED=$(cat "$CONTEXT_BUDGET_FILE" 2>/dev/null || echo 0)
  CTX_USED=$((CTX_USED / 2))
  echo "$CTX_USED" > "$CONTEXT_BUDGET_FILE"
fi
