...
### Scheduled Tasks API (#32)

DeepSeek Agent получил API для создания/управления cron задачами, как Claude Code `CronCreate`/`CronDelete`.

**Модуль:** `cron_manager.py` — управление пользовательскими cron задачами:
- `init_cron(scheduler, on_trigger)` — загрузка сохранённых задач в APScheduler
- `create_task(expression, action, session_id, desc)` — создание задачи (лимит: 50)
- `list_tasks()` — список с next_run
- `delete_task(tid)` — удаление задачи

**Хранение:** JSON в `/app/data/user_cron.json` (persist через restart). Валидация через `CronTrigger.from_crontab()`.

**API:**
- `GET /cron` — список задач
- `POST /cron` — создать задачу (body: expression, action, session_id, description)
