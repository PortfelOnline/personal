...
# Subagent Spawning Rules (token-conscious)
- **Explore субагент**: только когда нужно >3 поисковых запросов ИЛИ когда заранее не знаешь, как называется/где лежит. Для 1-2 точечных поисков — `Bash` + `grep`/`find` напрямую (≈10× дешевле спавна агента).
  - Хороший повод: «где определён символ X в незнакомом репо», «найди все места, где используется паттерн Y».
  - Плохой повод: «прочитай файл по известному пути», «сделай grep по одному слову».
- **graphify**: НЕ запускать на маленьких задачах «на пробу». Это тяжёлый pipeline (clone + extract + cluster + viz). Запускать только когда явно нужен knowledge graph целого репо/корпуса.
- **Параллельные субагенты**: спавнить параллельно только когда задачи реально независимы. Иначе последовательно — экономия на context preheat.
