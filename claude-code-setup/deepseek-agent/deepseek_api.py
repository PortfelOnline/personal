...
def route_model(prompt: str = "", system: str = "") -> str:
    """Auto-route between FAST_MODEL (flash) and DEFAULT_MODEL (pro).

    Flash (simple): short prompts, git questions, ls/cat, basic edits, typos
    Pro (complex): architecture, migrations, security, refactoring, long prompts
    """
    ...
    # Complex markers → pro
    complex_keywords = [
        "архитектур", "architecture", "рефактор", "refactor",
        "security", "безопасност", "migration", "миграц",
        "design pattern", "паттерн", "database schema", "ddl",
        "concurrency", "race condition", "memory leak", "deadlock",
        "performance profile", "оптимизац", "deploy", "release",
        "интеграц", "integration", "code review"
    ]

    for kw in complex_keywords:
        if kw in combined:
            return DEFAULT_MODEL  # pro

    # Very short → flash
    if len(prompt) < 150:
        return FAST_MODEL

    # Medium → flash for simple, pro for complex
    if len(prompt) < 800:
        flash_keywords = [
            "как дела", "покажи", "show", "ls", "cat", "git status",
            "что такое", "объясни", "спроси", "simple", "быстро",
            "fix typo", "исправь опечатку", "ренейм", "rename",
            "удали", "delete", "read", "list", "коммент"
        ]
        for kw in flash_keywords:
            if kw in combined:
                return FAST_MODEL
        return FAST_MODEL  # default simple

    # Long prompt > 2000 chars → pro
    if len(prompt) > 2000:
...
# ── Public API (для импорта в agent.py) ────────────────────────

def chat(messages=None, model=None, system=None, temperature=0.7,
         max_tokens=None, tools=None, stream=False, **kwargs):
    """Non-streaming chat — вызывает DeepSeek API и возвращает текст."""
    if model is None:
        model = route_model(
    ...
    # Build messages list in DeepSeek format
    full_messages = []
    if system:
    ...
    text, usage = _api_call(
        full_messages,
        model=model,
        stream=False,
        temperature=temperature,
        max_tokens=max_tokens or _resolve_max_tokens(
            count_messages(full_messages) if full_messages else 0,
            MAX_TOKENS_DEFAULT
        ),
        tools=tools,
        **kwargs
...
def stream_chat(messages=None, model=None, system=None, temperature=0.7,
                max_tokens=None, **kwargs):
    """Streaming chat — yields chunks from DeepSeek API."""
    if model is None:
        model = route_model(
            prompt=str(messages[-1]["content"]) if messages else "",
            system=system or ""
